                ...
                builder: (context, teslimatSnap) {
                  ...
                  return StreamBuilder<QuerySnapshot>(
                    stream: FirebaseFirestore.instance
                        ...
                        .where('firma', isEqualTo: currentFirmaName)
                        .snapshots(),
                    builder: (context, toplamaSnap) {
                      if (toplamaSnap.connectionState == ConnectionState.waiting) {
                        return const Center(child: CircularProgressIndicator());
                      }
                      ...
                      final allToplamalar = toplamaSnap.data?.docs ?? [];
                      ...
                      // Combined depot inputs
                      final List<Map<String, dynamic>> allDepotInputs = [];

                      for (var doc in companyDeliveries) {
                        final data = doc.data() as Map<String, dynamic>;
                        allDepotInputs.add({
                          'tarih': data['tarih'] as String? ?? '',
                          'miktar': (data['miktar'] as num?)?.toDouble() ?? 0.0,
                          'timestamp': data['timestamp'],
                        });
                      }

                      // 2. Add from toplamalar that went directly to center tanks
                      final companyDirectCollections = allToplamalar.where((doc) {
                        final d = doc.data() as Map<String, dynamic>;
                        final tank = d['tank'] as String? ?? '';
                        return tankNames.contains(tank);
                      }).toList();

                      for (var doc in companyDirectCollections) {
                        final data = doc.data() as Map<String, dynamic>;
                        allDepotInputs.add({
                          'tarih': data['tarih'] as String? ?? '',
                          'miktar': (data['m'] as num?)?.toDouble() ?? 0.0,
                          'timestamp': data['timestamp'],
                        });
                      }
                      ...
                      // Period filtering for combined depot inputs
                      final List<Map<String, dynamic>> periodDepotInputs = [];
                      for (var input in allDepotInputs) {
                        DateTime? date;
                        final dateStr = input['tarih'] as String? ?? '';
                        if (dateStr.isNotEmpty) {
                          try {
                            date = DateFormat('dd.MM.yyyy').parse(dateStr);
                          } catch (_) {
                        ...
                        if (date == null) {
                          final ts = input['timestamp'] as Timestamp?;
                          if (ts != null) {
                            date = ts.toDate();
                          }
                        }
                      ...
                      for (var input in allDepotInputs) {
                        final tarih = input['tarih'] as String? ?? '';
                        final miktar = input['miktar'] as double? ?? 0.0;
                        if (tarih.isNotEmpty) {
                      ...
                      for (var input in periodDepotInputs) {
                        totalGiris += input['miktar'] as double;
                      }
                      ...
                      return SingleChildScrollView(
                        child: Column(
                          ...
                          children: [
                            ...
                            (filteredSales.isEmpty && periodDepotInputs.isEmpty)
                                ? Center(
                                    child: Padding(
                                ...
                                : Builder(
                                    builder: (context) {
                                      ...
                                      // Add dates from periodDepotInputs
                                      for (var input in periodDepotInputs) {
                                        final String tarih = input['tarih'] ?? '';
